---
title: Conditional Statements
lessonId: java-conditionals
section: java
group: java-programming-logic
groupLabel: Programming Logic
groupOrder: 2
order: 1
difficulty: beginner
duration: 75 min
description: ""
---

### Choosing which lines run

A program often needs two different actions: drive if the path is clear, stop if it is not. An `if` statement runs a block only when a condition is true.

```java
int batteryLevel = 85;
if (batteryLevel > 50) {
    System.out.println("Battery is high enough to drive");
}
System.out.println("Check finished");
```

If `batteryLevel` were `20`, the inner print would not run. `Check finished` always runs.

### Code Blocks and Indentation

Conditional statements create **code blocks** enclosed by curly brackets `\{\}`. Code inside blocks should be indented 4 spaces or 1 tab to improve readability.

### Proper Indentation:

```java
int motorSpeed = 75;

// Correctly indented
if (motorSpeed > 50) {
    System.out.println("High speed mode");
    motorSpeed = 100;
}

// Incorrectly indented (avoid this)
if (motorSpeed > 50) {
motorSpeed = 100;
}
```

<RulesBox title="Comparison Operators:" subtitle="Essential Operators for Conditions:">

- `&gt;` - Greater than
- `&gt;=` - Greater than or equal to
- `&lt;` - Less than
- `&lt;=` - Less than or equal to
- `==` - Equal to
- `!=` - Not equal to

`=` assigns a value (`battery = 85`). `==` asks whether two values are equal (`battery == 85`). Using `=` inside `if (...)` is a compile error for `int` and a logic bug you do not want.

</RulesBox>

### Comparison Examples:

```java
int sensorReading = 255;

if (sensorReading != 0) {
    System.out.println("Sensor detected something");
}

if (sensorReading >= 200) {
    System.out.println("Strong signal detected");
}

if (sensorReading < 50) {
    System.out.println("Weak signal");
}
```

### Else Statements

The `else` statement provides an alternative code path when the if condition evaluates to false.

### If-Else Example:

```java
int robotPosition = 45;

if (robotPosition > 50) {
    System.out.println("Robot is in the forward zone");
} else {
    System.out.println("Robot is in the back zone");
}

// Another example
double batteryVoltage = 11.5;

if (batteryVoltage >= 12.0) {
    System.out.println("Battery voltage is good!");
} else {
    System.out.println("Battery needs charging!");
}
```

### Multiple Conditions: else if

Use `else if` to check multiple conditions in sequence. The first condition that evaluates to true will execute, and the rest will be skipped.

### Multiple Conditions:

```java
int gameScore = 85;

if (gameScore == 100) {
    System.out.println("Perfect score!");
} else if (gameScore >= 90) {
    System.out.println("Excellent performance!");
} else if (gameScore >= 70) {
    System.out.println("Good job!");
} else if (gameScore >= 50) {
    System.out.println("Keep practicing!");
} else {
    System.out.println("Need more work!");
}
```

<RulesBox title="Order of Execution:" subtitle="Important Rules:">

- Conditions are checked from **top to bottom**
- Only the **first true condition** executes
- Once a condition is true, **remaining conditions are skipped**
- Place **most specific conditions first**

</RulesBox>

### Execution Order Example:

```java
int number = 15;

// This works correctly
if (number % 3 == 0 && number % 5 == 0) {
    System.out.println("FizzBuzz");  // Divisible by both 3 and 5
} else if (number % 3 == 0) {
    System.out.println("Fizz");     // Divisible by 3 only
} else if (number % 5 == 0) {
    System.out.println("Buzz");     // Divisible by 5 only
} else {
    System.out.println(number);     // Not divisible by 3 or 5
}

// This would NOT work correctly:
// if (number % 3 == 0) would catch 15 first!
```

### Comparing Strings

Strings must be compared using the `.equals()` method, not the `==` operator. The `==` operator compares object references, not the actual text content.

### String Comparison:

```java
String robotStatus = "active";
String userInput = "ACTIVE";

// Correct way to compare strings
if (robotStatus.equals("active")) {
    System.out.println("Robot is running");
}

// Case-insensitive comparison
if (userInput.equalsIgnoreCase("active")) {
    System.out.println("User wants robot active");
}

// Comparing two string variables
String command1 = "forward";
String command2 = "backward";

if (command1.equals(command2)) {
    System.out.println("Commands are the same");
} else {
    System.out.println("Commands are different");
}

// Wrong way (don't do this)
if (robotStatus == "active") {
    // This may not work as expected!
}
```

### Logical Operators

Combine multiple conditions using logical operators: `&&` (AND), `||` (OR), and `!` (NOT).

<LogicalOperators title="Logical Operators:" subtitle="Combining Conditions:">

- `&&` - AND (both conditions must be true)
- `||` - OR (at least one condition must be true)
- `!` - NOT (reverses the boolean value)

#### Examples:

```java
// AND operator - both must be true
int speed = 50;
int temperature = 60;

if (speed >= 30 && temperature < 80) {
    System.out.println("Safe to operate");
}

// OR operator - at least one must be true
boolean buttonA = true;
boolean buttonB = false;

if (buttonA || buttonB) {
    System.out.println("A button was pressed");
}

// NOT operator - reverses the condition
boolean emergencyStop = false;

if (!emergencyStop) {
    System.out.println("Normal operation");
}
```

</LogicalOperators>

### Logical Operator Truth Table:

```java
// Truth table examples
boolean a = true;
boolean b = false;

System.out.println("=== AND (&&) Operation ===");
System.out.println("true && true = " + (true && true));   // true
System.out.println("true && false = " + (true && false)); // false
System.out.println("false && true = " + (false && true)); // false
System.out.println("false && false = " + (false && false)); // false

System.out.println("\n=== OR (||) Operation ===");
System.out.println("true || true = " + (true || true));   // true
System.out.println("true || false = " + (true || false)); // true
System.out.println("false || true = " + (false || true)); // true
System.out.println("false || false = " + (false || false)); // false

System.out.println("\n=== NOT (!) Operation ===");
System.out.println("!true = " + (!true));   // false
System.out.println("!false = " + (!false)); // true
```

When one value must match one of several exact options, [Switch Statements](/java/java-switch) is often clearer than a long `else if` chain.

<JavaPlayground id="java-conditionals" />

<ExerciseBox title="Try It Yourself:" subtitle="Exercise 1: Conditional Logic Practice" answers={[{"task":"Create a robot battery status checker with multiple levels","content":"public class BatteryStatusChecker {\n    public static void main(String[] args) {\n        double batteryVoltage = 11.8;\n        int batteryPercentage = 72;\n        \n        System.out.println(\"=== Battery Status Check ===\");\n        System.out.println(\"Voltage: \" + batteryVoltage + \"V\");\n        System.out.println(\"Percentage: \" + batteryPercentage + \"%\");\n        \n        // Multiple condition checking\n        if (batteryVoltage >= 12.5 && batteryPercentage >= 90) {\n            System.out.println(\"[GOOD] Battery Status: EXCELLENT\");\n            System.out.println(\"[OK] Ready for competition\");\n        } else if (batteryVoltage >= 12.0 && batteryPercentage >= 70) {\n            System.out.println(\"[MODERATE] Battery Status: GOOD\");\n            System.out.println(\"[OK] Safe for normal operation\");\n        } else if (batteryVoltage >= 11.5 && batteryPercentage >= 50) {\n            System.out.println(\"[MODERATE] Battery Status: MODERATE\");\n            System.out.println(\"[WARNING] Consider charging soon\");\n        } else if (batteryVoltage >= 11.0 && batteryPercentage >= 25) {\n            System.out.println(\"🟠 Battery Status: LOW\");\n            System.out.println(\"[WARNING] Charge immediately after match\");\n        } else {\n            System.out.println(\"[RED] Battery Status: CRITICAL\");\n            System.out.println(\"🛑 DO NOT OPERATE - CHARGE NOW\");\n        }\n    }\n}"},{"task":"Write a sensor range validator with proper error handling","content":"public class SensorValidator {\n    public static void main(String[] args) {\n        // Simulate different sensor readings\n        int colorSensorValue = 150;\n        double distanceSensorValue = 25.7;\n        int touchSensorValue = 1;\n        \n        System.out.println(\"=== Sensor Validation System ===\");\n        \n        // Color sensor validation (0-255 range)\n        if (colorSensorValue >= 0 && colorSensorValue <= 255) {\n            if (colorSensorValue > 200) {\n                System.out.println(\"[GOOD] Color Sensor: Bright light detected\");\n            } else if (colorSensorValue > 100) {\n                System.out.println(\"[MODERATE] Color Sensor: Medium light detected\");\n            } else {\n                System.out.println(\"[LOW] Color Sensor: Low light detected\");\n            }\n        } else {\n            System.out.println(\"[ERROR] Color Sensor: Invalid reading (\" + colorSensorValue + \")\");\n        }\n        \n        // Distance sensor validation (0-100 cm range)\n        if (distanceSensorValue >= 0.0 && distanceSensorValue <= 100.0) {\n            if (distanceSensorValue <= 5.0) {\n                System.out.println(\"[RED] Distance Sensor: OBSTACLE VERY CLOSE (\" + distanceSensorValue + \"cm)\");\n            } else if (distanceSensorValue <= 15.0) {\n                System.out.println(\"🟠 Distance Sensor: Obstacle close (\" + distanceSensorValue + \"cm)\");\n            } else {\n                System.out.println(\"[GOOD] Distance Sensor: Clear path (\" + distanceSensorValue + \"cm)\");\n            }\n        } else {\n            System.out.println(\"[ERROR] Distance Sensor: Out of range (\" + distanceSensorValue + \"cm)\");\n        }\n        \n        // Touch sensor validation (0 or 1)\n        if (touchSensorValue == 0 || touchSensorValue == 1) {\n            if (touchSensorValue == 1) {\n                System.out.println(\"[RED] Touch Sensor: PRESSED - Emergency stop!\");\n            } else {\n                System.out.println(\"[GOOD] Touch Sensor: Not pressed - Normal operation\");\n            }\n        } else {\n            System.out.println(\"[ERROR] Touch Sensor: Invalid state (\" + touchSensorValue + \")\");\n        }\n    }\n}"},{"task":"Implement a game mode selector using string comparisons","content":"public class GameModeSelector {\n    public static void main(String[] args) {\n        String currentMode = \"autonomous\";\n        String allianceColor = \"blue\";\n        int matchTime = 125; // seconds remaining\n        \n        System.out.println(\"=== FRC Game Mode System ===\");\n        System.out.println(\"Current Mode: \" + currentMode);\n        System.out.println(\"Alliance: \" + allianceColor);\n        System.out.println(\"Time Remaining: \" + matchTime + \"s\");\n        \n        // Game mode logic with string comparison\n        if (currentMode.equals(\"autonomous\")) {\n            System.out.println(\"[AUTO] AUTONOMOUS PERIOD\");\n            \n            if (allianceColor.equalsIgnoreCase(\"red\")) {\n                System.out.println(\"[RED] Red Alliance Strategy Active\");\n                System.out.println(\"   → Target red alliance scoring positions\");\n            } else if (allianceColor.equalsIgnoreCase(\"blue\")) {\n                System.out.println(\"[BLUE] Blue Alliance Strategy Active\");\n                System.out.println(\"   → Target blue alliance scoring positions\");\n            } else {\n                System.out.println(\"[ERROR] Unknown alliance color: \" + allianceColor);\n            }\n            \n        } else if (currentMode.equals(\"teleop\")) {\n            System.out.println(\"[TELEOP] TELEOPERATED PERIOD\");\n            System.out.println(\"   → Driver control enabled\");\n            System.out.println(\"   → Manual scoring mode\");\n            \n            if (matchTime <= 30) {\n                System.out.println(\"⏰ ENDGAME PERIOD - Final scoring opportunities!\");\n            }\n            \n        } else if (currentMode.equals(\"endgame\")) {\n            System.out.println(\"🏁 ENDGAME PERIOD\");\n            System.out.println(\"   → Focus on hanging/parking\");\n            System.out.println(\"   → Maximum point opportunities\");\n            \n        } else if (currentMode.equals(\"disabled\")) {\n            System.out.println(\"⏸️ ROBOT DISABLED\");\n            System.out.println(\"   → All motors stopped\");\n            System.out.println(\"   → Waiting for next period\");\n            \n        } else {\n            System.out.println(\"[ERROR] UNKNOWN MODE: \" + currentMode);\n            System.out.println(\"   → Defaulting to safe mode\");\n        }\n    }\n}"},{"task":"Build a complex safety system using logical operators","content":"public class RobotSafetySystem {\n    public static void main(String[] args) {\n        // System status variables\n        boolean emergencyStopPressed = false;\n        double motorTemperature = 68.5;\n        int batteryLevel = 45;\n        boolean communicationActive = true;\n        boolean sensorsResponding = true;\n        double robotSpeed = 0.75; // 0.0 to 1.0 scale\n        String operatorPresent = \"yes\";\n        \n        System.out.println(\"=== Robot Safety System Check ===\");\n        \n        // Critical safety checks using logical operators\n        boolean criticalFailure = emergencyStopPressed || \n                                 motorTemperature > 85 || \n                                 batteryLevel < 10 || \n                                 !communicationActive;\n        \n        boolean warningCondition = (batteryLevel < 25 && robotSpeed > 0.5) ||\n                                  (motorTemperature > 75 && robotSpeed > 0.8) ||\n                                  !sensorsResponding;\n        \n        boolean safeToOperate = !emergencyStopPressed && \n                               motorTemperature < 80 && \n                               batteryLevel > 15 && \n                               communicationActive && \n                               sensorsResponding &&\n                               operatorPresent.equalsIgnoreCase(\"yes\");\n        \n        // Safety decision logic\n        if (criticalFailure) {\n            System.out.println(\"🚨 CRITICAL FAILURE - IMMEDIATE SHUTDOWN\");\n            \n            if (emergencyStopPressed) {\n                System.out.println(\"   🛑 Emergency stop activated\");\n            }\n            if (motorTemperature > 85) {\n                System.out.println(\"   🌡️ Motor overheating: \" + motorTemperature + \"°C\");\n            }\n            if (batteryLevel < 10) {\n                System.out.println(\"   🔋 Critical battery level: \" + batteryLevel + \"%\");\n            }\n            if (!communicationActive) {\n                System.out.println(\"   📡 Communication failure\");\n            }\n            \n        } else if (warningCondition) {\n            System.out.println(\"[WARNING] WARNING CONDITIONS DETECTED\");\n            \n            if (batteryLevel < 25 && robotSpeed > 0.5) {\n                System.out.println(\"   🔋 Low battery with high speed operation\");\n            }\n            if (motorTemperature > 75 && robotSpeed > 0.8) {\n                System.out.println(\"   🌡️ High temperature with maximum speed\");\n            }\n            if (!sensorsResponding) {\n                System.out.println(\"   📊 Sensor communication issues\");\n            }\n            \n            System.out.println(\"   → Reducing robot performance for safety\");\n            \n        } else if (safeToOperate) {\n            System.out.println(\"[OK] ALL SYSTEMS NORMAL\");\n            System.out.println(\"   [GOOD] Robot cleared for full operation\");\n            \n            // Additional operational checks\n            if (batteryLevel > 80 && motorTemperature < 60) {\n                System.out.println(\"   ⭐ Optimal operating conditions\");\n            }\n            \n        } else {\n            System.out.println(\"❓ UNKNOWN SAFETY STATE\");\n            System.out.println(\"   → Defaulting to safe mode\");\n        }\n        \n        // Status summary\n        System.out.println(\"\\n=== System Status Summary ===\");\n        System.out.println(\"Emergency Stop: \" + (emergencyStopPressed ? \"PRESSED\" : \"Normal\"));\n        System.out.println(\"Motor Temp: \" + motorTemperature + \"°C\");\n        System.out.println(\"Battery: \" + batteryLevel + \"%\");\n        System.out.println(\"Communication: \" + (communicationActive ? \"Active\" : \"Failed\"));\n        System.out.println(\"Sensors: \" + (sensorsResponding ? \"Responding\" : \"Error\"));\n        System.out.println(\"Operator Present: \" + operatorPresent);\n    }\n}"}]}>

Fill in each starter, click Run, then open Show Answers when you want to compare.

- Create a robot battery status checker with multiple levels

```java
public class Main {
    public static void main(String[] args) {
        double batteryVoltage = 11.8;
        int batteryPercentage = 72;

        System.out.println("Voltage: " + batteryVoltage + "V");
        System.out.println("Percentage: " + batteryPercentage + "%");

        // TODO: print a status using if / else if / else
        // Use both voltage and percentage
    }
}
```

- Write a sensor range validator with proper error handling

```java
public class Main {
    public static void main(String[] args) {
        int colorSensorValue = 150;
        double distanceSensorValue = 25.7;
        int touchSensorValue = 1;

        System.out.println("Color: " + colorSensorValue);
        System.out.println("Distance: " + distanceSensorValue + " cm");
        System.out.println("Touch: " + touchSensorValue);

        // TODO: check that each reading is in range, then print a message
        // Color: 0-255. Distance: 0-100 cm. Touch: 0 or 1.
    }
}
```

- Implement a game mode selector using string comparisons

```java
public class Main {
    public static void main(String[] args) {
        String currentMode = "autonomous";
        String allianceColor = "blue";
        int matchTime = 125;

        System.out.println("Mode: " + currentMode);
        System.out.println("Alliance: " + allianceColor);
        System.out.println("Time remaining: " + matchTime + "s");

        // TODO: compare strings with .equals() and .equalsIgnoreCase()
        // Modes include autonomous, teleop, endgame, and disabled
    }
}
```

- Build a complex safety system using logical operators

```java
public class Main {
    public static void main(String[] args) {
        boolean emergencyStopPressed = false;
        double motorTemperature = 68.5;
        int batteryLevel = 45;
        boolean communicationActive = true;
        boolean sensorsResponding = true;
        double robotSpeed = 0.75;
        String operatorPresent = "yes";

        System.out.println("E-stop: " + emergencyStopPressed);
        System.out.println("Motor temp: " + motorTemperature);
        System.out.println("Battery: " + batteryLevel + "%");
        System.out.println("Comms: " + communicationActive);
        System.out.println("Sensors: " + sensorsResponding);
        System.out.println("Speed: " + robotSpeed);
        System.out.println("Operator: " + operatorPresent);

        // TODO: combine these with &&, ||, and ! and print the safety result
    }
}
```

</ExerciseBox>

