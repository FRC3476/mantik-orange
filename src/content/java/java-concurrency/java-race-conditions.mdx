...
difficulty: advanced
duration: 35 min
description: How two threads corrupt shared data, what synchronized and volatile actually guarantee, and why avoiding shared state beats protecting it.
---

<RulesBox title="Optional later">

Same caution as [Threads](/java/java-threads-basics): prefer sharing nothing with the robot loop. Locking around motors is usually the wrong instinct. A vision thread updating a pose field is a real case.
...
class Tracker {
    ...
    void expensiveWorkNeedingNoLock() {
        // pretend this takes time; it does not need the lock
    }
...
class Worker {
    ...
    void run() {
        while (running && steps < 80) {     // bound so the demo always ends
            doWork();
        }
...
<ContentTable
  ...
  rows={[
    ...
    ["Nothing genuinely shared", "Nothing &mdash; the best option"]
  ]}
/>
...
public class Main {
    ...
    static void increment() {
        // TODO: make this synchronized and increment count
    }
