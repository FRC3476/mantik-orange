...
<RulesBox title="This page is optional">

This course's swerve drivetrain is Tuner X `CommandSwerveDrivetrain`. Phoenix already owns the motors, odometry thread, and simulation notifier. You do not wrap that generated drivetrain in `ModuleIO`.

This lesson is the replay pattern for subsystems you write yourself, or for teams that later adopt [AdvantageKit](/frc/advantagekit). The `ModuleIO` samples below are an **alternative AdvantageKit swerve architecture**. They do not plug into the generated drivetrain.
...
- **`XxxIOSomething`** — one implementation per hardware or environment: `ModuleIOTalonFX`, `ModuleIOSim`, `VisionIOPhotonVision`, `VisionIOLimelight`.
- **`Xxx`** — the subsystem, which holds an `XxxIO` and never names a motor class.

</RulesBox>

This pattern comes from Team 6328's AdvantageKit template. Use it on mechanisms you own. Do not retrofit it onto Tuner X `CommandSwerveDrivetrain`.

### The Interface

The `ModuleIO` below is that **alternative AdvantageKit swerve** surface. It is nothing but data and empty defaults:
...
This REAL / SIM / REPLAY block belongs only to the AdvantageKit swerve alternative. A Tuner X project constructs `TunerConstants.createDrivetrain()` once and does not branch on `ModuleIO`.
...
`Drive` is constructed identically in all three branches. It has no mode flag and no `if (isSimulation())` — everything mode-specific is in which object gets passed in. See [Simulation Basics](/frc/simulation-basics) for what each mode does.
...
<ContentTable
  ...
  rows={[
    ["Course swerve (Tuner X)", "No", "Phoenix already owns motors, odometry, and sim &mdash; do not wrap it in ModuleIO"],
    ["AdvantageKit swerve (alternative)", "Yes", "Only if you adopt that architecture instead of CommandSwerveDrivetrain"],
...
For a conveyor that runs one motor at one duty cycle, the abstraction usually costs more than it returns. Use `@AutoLogOutput` on that subsystem's getters and setters instead — see [Subsystems](/frc/subsystems) — and accept that its outputs are logged but its inputs are not replayed.
...
<LinkGrid links={[{"label":"Subsystems","id":"subsystems"},{"label":"AdvantageKit","id":"advantagekit"},{"label":"Simulation Basics","id":"simulation-basics"},{"label":"Swerve Drive","id":"swerve-drive-example"},{"label":"AdvantageKit IO Interfaces","url":"https://docs.advantagekit.org/data-flow/recording-inputs/io-interfaces/"}]} title="Resources" section="frc" />
