...
**Vision-Based (Camera-Based):** Uses cameras to observe field features (AprilTags, field elements, game pieces) and calculate pose from these observations. A correct tag solve is an absolute field measurement, but it is not guaranteed: tags can be blocked, poorly calibrated, or ambiguous. Vision updates are typically slower than odometry.
