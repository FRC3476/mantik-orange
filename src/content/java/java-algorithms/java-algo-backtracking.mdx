...
Prerequisites: [Recursion](/java/java-algo-recursion) and [Depth-First Search](/java/java-algo-dfs). You will see this in programming challenges, not in teleop.
...
<ContentTable
  ...
  rows={[
    ["Base case", "All decisions made &mdash; record or evaluate the result"],
...
public static void main(String[] args) {
    int[] values = {1, 2, 3};
    permute(values, new boolean[values.length]);
    System.out.println(results);
}
```
...
public static void main(String[] args) {
    System.out.println(canReachTarget(new int[]{3, 1, 7}, 0, 0, 8));
}
```
...
<RulesBox title="A prune must actually be sound">
...
public static void main(String[] args) {
    int n = 4;
    System.out.println(countArrangements(n, 0, new boolean[n]));
}
```
...
If the bound is large, backtracking will not finish, and the intended solution is probably [dynamic programming](/java/java-algo-memoization) or something greedy.
...
public class Main {
    static List<Integer> current = new ArrayList<Integer>();
...
public class Main {
    static List<Character> current = new ArrayList<Character>();
    static List<List<Character>> results = new ArrayList<List<Character>>();

    static void permute(char[] letters, boolean[] used) {
    }

    public static void main(String[] args) {
        char[] letters = {'A', 'B', 'C', 'D'};
        permute(letters, new boolean[letters.length]);
        System.out.println(results);
    }
}
```
...
public class Main {
    ...
    public static void main(String[] args) {
        int[] values = {3, 1, 7, 2, 8};
        System.out.println(canReachTarget(values, 0, 0, 8));
    }
}
```
...
public class Main {
    ...
    public static void main(String[] args) {
        int n = 4; // try up to 8 after it works
        System.out.println(countArrangements(n, 0, new boolean[n]));
    }
}
```
