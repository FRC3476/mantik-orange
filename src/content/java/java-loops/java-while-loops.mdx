...
The `condition` is checked before each pass. If it is true, the block runs again. If it is false, the program continues after the loop.
...
A `while (true)` loop with no `break` never stops. Do not run this as-is in the browser.
...
Stop the loop with `break` when a condition is met:
...
public class Main {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            // TODO: handle a reading and print battery status
        }
    }
}
```
...
public class Main {
    public static void main(String[] args) {
        ...
        while (true) {
            System.out.println("Guess a number 1-100 (quit to stop):");
            String input = scanner.nextLine();
            ...
            // TODO: parse input, reject invalid guesses with continue, compare to target
        }
    }
}
```
...
public class Main {
    public static void main(String[] args) {
        ...
        while (readingCount < MAX_READINGS) {
            readingCount++;
            // TODO: move current toward target; stop when close enough
        }
    }
}
```
