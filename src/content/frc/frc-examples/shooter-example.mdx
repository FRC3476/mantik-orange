...
public Command getShootSequence() {
    return new SequentialCommandGroup(
        // Intended speed: 4000 RPM. Talon FX setSpeed() in this lesson takes RPS.
        // 4000 / 60 = 66.67 RPS. Keep setSpeed and isReady in the same units.
        // SPARK MAX tab: setSpeed/isReady use RPM, so pass 4000 there instead.
        final double targetRps = 4000.0 / 60.0;
        new InstantCommand(() -> m_shooter.setSpeed(targetRps), m_shooter),
        new WaitUntilCommand(() -> m_shooter.isReady(targetRps)),
...
Once velocity control holds a speed, [Shooter Shot Table](/frc/shooter-shot-table) selects flywheel **output RPS** and hood angle from field pose. That lesson converts those RPS values to Talon FX rotor units with an explicit gear ratio. This page's `setSpeed` still talks to the motor in the units of each vendor tab.

<LinkGrid links={[{"label":"Shooter Shot Table","id":"shooter-shot-table"},{"label":"Elevator Example","id":"elevator-example"},{"label":"PID Control","id":"frc-pid-control"},{"label":"WPILib: Feedforward Control (Practice)","url":"https://docs.wpilib.org/en/stable/docs/software/advanced-controls/controllers/feedforward.html"}]} title="Resources" section="frc" />
