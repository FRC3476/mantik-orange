...
This is a literal [stack](/java/java-ds-stacks) — the same last-in-first-out structure — which is why running out of space is a `StackOverflowError`.
...
- **The data structure is recursive.** A [tree](/java/java-ds-trees) node contains subtrees, which are trees. Recursion mirrors the shape.
...
Two recursive calls, and the structure handles the bookkeeping. Most tree and graph work looks like this — see [Tree Traversals](/java/java-algo-tree-traversals) and [Depth-First Search](/java/java-algo-dfs).
...
This is correct but recomputes the same values constantly — `fib(40)` makes over a billion calls. The fix is to remember results you have already computed, covered in [Memoization and Dynamic Programming](/java/java-algo-memoization).
...
public static void main(String[] args) {
    System.out.println(countMatches(new int[]{1, 4, 4, 2, 4}, 4));
}
```
...
- **Modifying shared state without undoing it.** See [Backtracking](/java/java-algo-backtracking).
...
```java
public class Main {
    static void printSubsets(int[] values) {
        // TODO: print every subset
    }
