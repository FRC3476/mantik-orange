---
title: Passing Functions as Data
lessonId: java-fn-functions-as-data
section: java
group: java-functions-as-data
groupLabel: "Functions as Data"
groupOrder: 7
order: 1
difficulty: intermediate
duration: 30 min
description: Treating behaviour as a value you can store and pass around, why Java needs an interface to do it, and what problem it solves.
---

<RulesBox title="Stretch — reading team and WPILib code">

This module is how Java treats a piece of behavior as a value. You will see it in button bindings and `until` conditions. You are not expected to invent new functional interfaces on the robot. You need [Interfaces](/java/java-interfaces) first.

</RulesBox>

### The repetition problem

Suppose you need to filter a list of sensor readings several different ways.

```java
static List<Integer> above100(List<Integer> readings) {
    List<Integer> result = new ArrayList<>();
    for (int r : readings) {
        if (r > 100) result.add(r);
    }
    return result;
}

static List<Integer> evenOnly(List<Integer> readings) {
    List<Integer> result = new ArrayList<>();
    for (int r : readings) {
        if (r % 2 == 0) result.add(r);
    }
    return result;
}

public static void main(String[] args) {
    List<Integer> readings = Arrays.asList(40, 80, 120, 160);
    System.out.println(above100(readings));
    System.out.println(evenOnly(readings));
}
```

These methods are identical except for one line. Everything else — creating the list, looping, adding, returning — is duplicated.

You already know how to pass a *value* into a method to avoid repetition. What you need here is to pass the **test itself**.

### Behaviour as a parameter

Java has no standalone function type. What it has is interfaces, and an object implementing a one-method interface serves the same purpose.

```java
interface ReadingTest {
    boolean matches(int reading);
}
```

Now the loop is written once, and the differing line becomes a parameter:

```java
class Main {
    interface ReadingTest {
        boolean matches(int reading);
    }

    static List<Integer> filter(List<Integer> readings, ReadingTest test) {
        List<Integer> result = new ArrayList<>();
        for (int r : readings) {
            if (test.matches(r)) {      // the caller decides what this means
                result.add(r);
            }
        }
        return result;
    }

    public static void main(String[] args) {
        List<Integer> readings = Arrays.asList(40, 80, 120, 160);
        List<Integer> high = filter(readings, new ReadingTest() {
            @Override
            public boolean matches(int reading) {
                return reading > 100;
            }
        });
        System.out.println(high);
    }
}
```

Calling it means supplying an implementation:

```java
class Main {
    interface ReadingTest {
        boolean matches(int reading);
    }

    static List<Integer> filter(List<Integer> readings, ReadingTest test) {
        List<Integer> result = new ArrayList<>();
        for (int r : readings) {
            if (test.matches(r)) result.add(r);
        }
        return result;
    }

    public static void main(String[] args) {
        List<Integer> readings = Arrays.asList(40, 80, 120, 160);

        ReadingTest above100 = new ReadingTest() {
            @Override
            public boolean matches(int reading) {
                return reading > 100;
            }
        };

        List<Integer> high = filter(readings, above100);
        System.out.println(high);
    }
}
```

That block is an **anonymous class** — a class with no name, defined and instantiated in one expression. It is verbose, and [Lambda Expressions](/java/java-fn-lambdas) exist precisely to shorten it. But this is what is really happening underneath, and seeing it once makes lambdas much less mysterious.

<RulesBox title="Why an interface is required">

In some languages you can pass a function directly. In Java, the thing you pass is always an object. The interface gives that object a type, and [runtime polymorphism](/java/java-runtime-polymorphism) picks the right `matches` implementation when `filter` calls it.

So "passing a function" in Java is really "passing an object with one interesting method". Every lambda you write is compiled into something along these lines.

</RulesBox>

### What this buys you

The `filter` method now works with tests that did not exist when it was written:

```java
class Main {
    interface ReadingTest {
        boolean matches(int reading);
    }

    static List<Integer> filter(List<Integer> readings, ReadingTest test) {
        List<Integer> result = new ArrayList<>();
        for (int r : readings) {
            if (test.matches(r)) result.add(r);
        }
        return result;
    }

    public static void main(String[] args) {
        List<Integer> readings = Arrays.asList(40, 80, 120, 160);

        List<Integer> even = filter(readings, new ReadingTest() {
            @Override public boolean matches(int r) { return r % 2 == 0; }
        });

        List<Integer> inRange = filter(readings, new ReadingTest() {
            @Override public boolean matches(int r) { return r >= 50 && r <= 150; }
        });

        System.out.println(even);
        System.out.println(inRange);
    }
}
```

One loop, any number of behaviours. The pattern generalises beyond filtering — anywhere you find near-identical methods differing by a line or two, the difference can usually become a parameter.

### Functional interfaces

An interface with exactly one abstract method is called a **functional interface**, and it is the shape Java uses for this everywhere.

```java
class Main {
    @FunctionalInterface
    interface ReadingTest {
        boolean matches(int reading);
    }

    public static void main(String[] args) {
        ReadingTest above100 = new ReadingTest() {
            @Override
            public boolean matches(int reading) {
                return reading > 100;
            }
        };
        System.out.println(above100.matches(150));
    }
}
```

The annotation is optional but worth adding. It makes the compiler reject the interface if someone later adds a second abstract method — which would silently break every lambda written against it.

Java already ships the common shapes, so you rarely define your own. Those are covered in [Supplier and the Functional Interfaces](/java/java-fn-supplier).

### Deferring work

There is a second use, distinct from removing duplication: describing work now and running it later.

```java
interface Action {
    void run();
}

class ActionQueue {
    private final Queue<Action> pending = new ArrayDeque<>();

    void schedule(Action action) {
        pending.offer(action);           // stored, not executed
    }

    void runAll() {
        while (!pending.isEmpty()) {
            pending.poll().run();        // executed now
        }
    }
}

ActionQueue queue = new ActionQueue();
queue.schedule(new Action() {
    @Override public void run() { System.out.println("first"); }
});
queue.schedule(new Action() {
    @Override public void run() { System.out.println("second"); }
});
queue.runAll();
```

The action is created at one point in the program and executed at another. That separation is the foundation of event handlers, callbacks, and the [Command Pattern](/java/java-dp-command) — and it is how FRC's command-based framework schedules robot behaviour.

<RulesBox title="Two distinct motivations">

- **Removing duplication** — the surrounding code is fixed, one step varies. Pass the varying step in.
- **Deferring execution** — you know what should happen but not yet when. Store the behaviour and run it later.

Both use the same mechanism. Recognising which one you need helps you name things sensibly.

</RulesBox>

### Returning behaviour

A method can also hand back an implementation, which lets you build behaviour from parameters:

```java
class Main {
    interface ReadingTest {
        boolean matches(int reading);
    }

    static List<Integer> filter(List<Integer> readings, ReadingTest test) {
        List<Integer> result = new ArrayList<>();
        for (int r : readings) {
            if (test.matches(r)) result.add(r);
        }
        return result;
    }

    static ReadingTest greaterThan(int threshold) {
        return new ReadingTest() {
            @Override
            public boolean matches(int reading) {
                return reading > threshold;      // captures threshold
            }
        };
    }

    public static void main(String[] args) {
        List<Integer> readings = Arrays.asList(40, 80, 120, 160, 600);
        List<Integer> high = filter(readings, greaterThan(100));
        List<Integer> veryHigh = filter(readings, greaterThan(500));
        System.out.println(high);
        System.out.println(veryHigh);
    }
}
```

The returned object remembers `threshold` — a captured variable. Note the restriction: a captured local variable must be effectively final, meaning you cannot reassign it after capture. Java enforces this at compile time.

<RulesBox title="Common mistakes">

- **Adding a second abstract method** to a functional interface, breaking every lambda using it. `@FunctionalInterface` prevents this.
- **Reassigning a captured local variable**, which is a compile error.
- **Calling the behaviour when you meant to store it.** `schedule(action)` stores; `action.run()` executes.
- **Over-applying the pattern.** If a method has exactly one caller and one behaviour, parameterising it adds indirection for no benefit.

</RulesBox>

<JavaPlayground id="java-fn-functions-as-data" />

<ExerciseBox title="Practice" description="Use anonymous classes throughout — lambdas come in the next lesson.">

Fill in each starter, click Run, then open Hints if you get stuck.

1. Define a `ReadingTest` interface and a `filter` method, then filter a list three different ways.

```java
import java.util.*;

public class Main {
    interface ReadingTest {
        boolean matches(int reading);
    }

    static List<Integer> filter(List<Integer> readings, ReadingTest test) {
        // TODO: keep readings that match
        return new ArrayList<Integer>();
    }

    public static void main(String[] args) {
        List<Integer> readings = new ArrayList<Integer>();
        readings.add(40);
        readings.add(80);
        readings.add(120);
        readings.add(160);

        ReadingTest above100 = new ReadingTest() {
            public boolean matches(int r) {
                return false;
            }
        };
        ReadingTest even = new ReadingTest() {
            public boolean matches(int r) {
                return false;
            }
        };
        ReadingTest inRange = new ReadingTest() {
            public boolean matches(int r) {
                return false;
            }
        };

        System.out.println(filter(readings, above100));
        System.out.println(filter(readings, even));
        System.out.println(filter(readings, inRange));
    }
}
```

2. Define a `Transformer` interface with `int apply(int value)` and write a `map` method that applies it to every element of a list.

```java
import java.util.*;

public class Main {
    interface Transformer {
        int apply(int value);
    }

    static List<Integer> map(List<Integer> readings, Transformer transformer) {
        // TODO: apply to every element
        return readings;
    }

    public static void main(String[] args) {
        List<Integer> readings = new ArrayList<Integer>();
        readings.add(40);
        readings.add(80);
        readings.add(120);
        readings.add(160);

        Transformer doubled = new Transformer() {
            public int apply(int value) {
                return value;
            }
        };

        System.out.println(map(readings, doubled));
    }
}
```

3. Write a `greaterThan(int)` method that returns a `ReadingTest`, and use it twice with different thresholds.

```java
import java.util.*;

public class Main {
    interface ReadingTest {
        boolean matches(int reading);
    }

    static List<Integer> filter(List<Integer> readings, ReadingTest test) {
        // TODO: keep readings that match
        return new ArrayList<Integer>();
    }

    static ReadingTest greaterThan(int threshold) {
        return new ReadingTest() {
            public boolean matches(int reading) {
                return false;
            }
        };
    }

    public static void main(String[] args) {
        List<Integer> readings = new ArrayList<Integer>();
        readings.add(40);
        readings.add(80);
        readings.add(120);
        readings.add(160);
        readings.add(600);

        System.out.println(filter(readings, greaterThan(100)));
        System.out.println(filter(readings, greaterThan(500)));
    }
}
```

4. Define an `Action` interface and an `ActionQueue` that stores actions and runs them in order.

```java
import java.util.*;

public class Main {
    interface Action {
        void run();
    }

    static class ActionQueue {
        private final Queue<Action> pending = new ArrayDeque<Action>();

        void schedule(Action action) {
            // TODO: store, do not run yet
        }

        void runAll() {
            // TODO: poll and run in order
        }
    }

    public static void main(String[] args) {
        ActionQueue queue = new ActionQueue();
        queue.schedule(new Action() {
            public void run() {
                System.out.println("first");
            }
        });
        queue.schedule(new Action() {
            public void run() {
                System.out.println("second");
            }
        });
        queue.runAll();
    }
}
```

5. Write a `sort` helper that takes a list and a comparison interface you define yourself, so the caller decides the ordering.

```java
import java.util.*;

public class Main {
    interface Comparison {
        int compare(int a, int b);
    }

    static void sort(List<Integer> values, Comparison order) {
        // TODO: sort using order.compare
    }

    public static void main(String[] args) {
        List<Integer> values = new ArrayList<Integer>();
        values.add(160);
        values.add(40);
        values.add(120);
        values.add(80);

        Comparison ascending = new Comparison() {
            public int compare(int a, int b) {
                return 0;
            }
        };

        sort(values, ascending);
        System.out.println(values);
    }
}
```

<details>
<summary>Hints</summary>

1. As written above.
2. Same loop shape; call `apply` and add the result.
3. The returned anonymous class captures the parameter.
4. Store in an `ArrayDeque`; `runAll` polls and calls `run()`.
5. Your interface needs a method returning a negative number, zero, or a positive number. Then any simple sort works using it for comparisons. This is exactly how `Comparator` is built.

</details>

</ExerciseBox>

<LinkGrid
  title="Next"
  section="java"
  links={[
    {"label":"Lambda Expressions and Syntactic Sugar","id":"java-fn-lambdas"},
    {"label":"Supplier and the Functional Interfaces","id":"java-fn-supplier"}
  ]}
/>
