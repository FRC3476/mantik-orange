...
```java
class Motor {
    void set(double power) { System.out.println("motor power: " + power); }
}
...
class Camera implements Stoppable {          // unrelated to Subsystem
    void stopRecording() { System.out.println("camera stopped"); }
    @Override
    public void stop() {
        stopRecording();
    }
...
class Intake implements Stoppable {
    public void stop() { System.out.println("intake stopped"); }
}

class Camera implements Stoppable {
    public void stop() { System.out.println("camera stopped"); }
}

Stoppable intake = new Intake();
Stoppable camera = new Camera();
List<Stoppable> everything = Arrays.asList(intake, camera);
for (Stoppable s : everything) {
    s.stop();
}
```

That is the thing [inheritance](/java/java-inheritance) cannot do. A class extends one parent, but implements as many interfaces as it needs.
...
class Motor {
    ...
    void set(double power) { this.power = power; }
...
intake.stop();
System.out.println(intake.toLogLine());
```
...
When shared behaviour needs shared *data*, you still want an [abstract class](/java/java-abstract-classes).
...
List<Stoppable> everything = Arrays.asList(new Intake(), new Camera());
...
An interface with exactly one abstract method can be implemented by a [lambda](/java/java-fn-lambdas):
...
System.out.println("150 matches? " + above100.matches(150));
System.out.println("50 matches? " + above100.matches(50));
```
...
This is the mechanism behind the whole [Functions as Data](/java/java-fn-functions-as-data) module.
...
<ContentTable
  ...
  rows={[
    ...
    ["Both would work", "Interface &mdash; it costs nothing to add later"]
  ]}
/>
...
The same reasoning drives [dependency injection](/java/java-dp-dependency-injection) — accept an interface, and callers decide what to supply, including a fake for testing.
...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("looped " + everything.size());
    }
}
```
...
class Intake implements Stoppable, Loggable {
    @Override
    public void stop() {
        // TODO
    }
...
public class Main {
    public static void main(String[] args) {
        Intake intake = new Intake();
        Stoppable asStoppable = intake;
        Loggable asLoggable = intake;
        asStoppable.stop();
        System.out.println(asLoggable.toLogLine());
    }
}
```
...
class Intake implements Stoppable {
    ...
    // TODO: override emergencyStop in only one class
}
...
class HardwareMotor implements Motor {
    ...
    public void set(double power) {
        System.out.println("hardware: " + power);
    }
}
...
class Intake {
    private HardwareMotor motor = new HardwareMotor();
...
public class Main {
    public static void main(String[] args) {
        ...
        Intake intake = new Intake();
        intake.stop();
        System.out.println("last power: " + fake.lastPower);
        // TODO: construct Intake with the fake instead
    }
}
```
