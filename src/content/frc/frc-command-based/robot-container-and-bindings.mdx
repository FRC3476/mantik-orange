...
Subsystems that use an [IO layer](/frc/io-layer) are constructed differently depending on the runtime mode, and `RobotContainer` is the only place that makes that choice:
...
Adding characterization routines as chooser options is a convenient way to run them from the dashboard without a separate code path. See [Tunable Constants and Characterization](/frc/tunable-constants).
