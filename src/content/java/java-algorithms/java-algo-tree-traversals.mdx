...
Finish [Trees](/java/java-ds-trees) and [Recursion](/java/java-algo-recursion) first. This is the full walk of the four orders, not a new data structure.
...
Assume the node type from [Trees](/java/java-ds-trees):
...
The odd one out. It cannot be done with plain recursion because it moves across the tree rather than down it. Use a [queue](/java/java-ds-queues):
...
This is [breadth-first search](/java/java-algo-bfs) applied to a tree.
...
Recursion is clearer, but a very deep tree can overflow the call stack. An explicit [stack](/java/java-ds-stacks) removes that limit.
