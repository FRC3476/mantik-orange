---
title: Depth-First Search
lessonId: java-algo-dfs
section: java
group: java-algorithms
groupLabel: "Common Algorithms"
groupOrder: 6
order: 5
difficulty: intermediate
duration: 30 min
description: Following one path as far as it goes before backing up, tracking visited nodes so cycles do not trap you, and the questions DFS answers well.
---

<RulesBox title="Optional later">

[Graphs](/java/java-ds-graphs) already sketched this. This lesson is the tool, including `visited`. You also need [Recursion](/java/java-algo-recursion).

</RulesBox>

### Going deep first

Depth-first search explores by committing to one direction and following it until it dead-ends, then backing up and trying the next option.

On a [tree](/java/java-algo-tree-traversals) this is exactly pre-order traversal. On a graph one thing changes: a graph can contain cycles, so you must remember where you have been.

```java
static void dfs(int node, List<List<Integer>> adj, boolean[] visited) {
    visited[node] = true;
    System.out.println("visiting " + node);

    for (int neighbour : adj.get(node)) {
        if (!visited[neighbour]) {
            dfs(neighbour, adj, visited);
        }
    }
}

public static void main(String[] args) {
    List<List<Integer>> adj = Arrays.asList(
        Arrays.asList(1, 2),
        Arrays.asList(0, 3),
        Arrays.asList(0),
        Arrays.asList(1)
    );
    dfs(0, adj, new boolean[4]);
}
```

<RulesBox title="Mark visited before recursing, not after">

```java norun
visited[node] = true;      // first line of the method
for (...) { ... }
```

If you mark the node after the loop, a cycle sends the search back into a node still marked unvisited, and it recurses forever.

This single line is the difference between DFS on a graph and DFS on a tree. Trees have no cycles, so they need no `visited` array. Graphs always do.

</RulesBox>

### Counting connected components

The classic use. Start a search from every node not yet reached; each fresh start is one separate group.

```java
static void dfs(int node, List<List<Integer>> adj, boolean[] visited) {
    visited[node] = true;
    for (int neighbour : adj.get(node)) {
        if (!visited[neighbour]) {
            dfs(neighbour, adj, visited);
        }
    }
}

static int countComponents(List<List<Integer>> adj, int n) {
    boolean[] visited = new boolean[n];
    int components = 0;

    for (int i = 0; i < n; i++) {
        if (!visited[i]) {
            components++;
            dfs(i, adj, visited);
        }
    }
    return components;
}

public static void main(String[] args) {
    int n = 4;
    List<List<Integer>> adj = Arrays.asList(
        Arrays.asList(1),
        Arrays.asList(0),
        Arrays.asList(3),
        Arrays.asList(2)
    );
    System.out.println(countComponents(adj, n));
}
```

The outer loop matters. Searching only from node 0 finds one group and silently misses every disconnected part.

### Grid flood fill

A grid is a graph where cells are nodes and adjacent cells are edges. Counting regions of connected cells is the same algorithm:

```java
static final int[] DR = {-1, 1, 0, 0};
static final int[] DC = {0, 0, -1, 1};

static void fill(char[][] grid, boolean[][] seen, int r, int c) {
    seen[r][c] = true;

    for (int d = 0; d < 4; d++) {
        int nr = r + DR[d];
        int nc = c + DC[d];

        if (nr < 0 || nr >= grid.length) continue;
        if (nc < 0 || nc >= grid[0].length) continue;
        if (seen[nr][nc] || grid[nr][nc] == '#') continue;

        fill(grid, seen, nr, nc);
    }
}

public static void main(String[] args) {
    char[][] grid = {
        {'.', '.', '#'},
        {'#', '.', '.'},
        {'.', '#', '.'}
    };
    boolean[][] seen = new boolean[grid.length][grid[0].length];
    fill(grid, seen, 0, 0);
    System.out.println(seen[0][1]);   // true — same region as (0, 0)
}
```

The bounds checks must come **before** reading `grid[nr][nc]`. Java stops evaluating at the first `continue`, so an out-of-range index is never used. Reorder them and you get `ArrayIndexOutOfBoundsException`.

Counting regions then follows the same shape as components:

```java
static final int[] DR = {-1, 1, 0, 0};
static final int[] DC = {0, 0, -1, 1};

static void fill(char[][] grid, boolean[][] seen, int r, int c) {
    seen[r][c] = true;
    for (int d = 0; d < 4; d++) {
        int nr = r + DR[d];
        int nc = c + DC[d];
        if (nr < 0 || nr >= grid.length) continue;
        if (nc < 0 || nc >= grid[0].length) continue;
        if (seen[nr][nc] || grid[nr][nc] == '#') continue;
        fill(grid, seen, nr, nc);
    }
}

public static void main(String[] args) {
    char[][] grid = {
        {'.', '.', '#'},
        {'#', '.', '.'},
        {'.', '#', '.'}
    };
    boolean[][] seen = new boolean[grid.length][grid[0].length];

    int regions = 0;
    for (int r = 0; r < grid.length; r++) {
        for (int c = 0; c < grid[0].length; c++) {
            if (grid[r][c] == '.' && !seen[r][c]) {
                regions++;
                fill(grid, seen, r, c);
            }
        }
    }
    System.out.println(regions);
}
```

### Detecting a cycle

In an **undirected** graph, a cycle exists if you reach an already-visited node that is not the one you came from:

```java
static boolean hasCycle(int node, int parent, List<List<Integer>> adj, boolean[] visited) {
    visited[node] = true;

    for (int neighbour : adj.get(node)) {
        if (!visited[neighbour]) {
            if (hasCycle(neighbour, node, adj, visited)) return true;
        } else if (neighbour != parent) {
            return true;                  // reached a visited node another way
        }
    }
    return false;
}

public static void main(String[] args) {
    List<List<Integer>> adj = Arrays.asList(
        Arrays.asList(1, 2),
        Arrays.asList(0, 2),
        Arrays.asList(0, 1)
    );
    System.out.println(hasCycle(0, -1, adj, new boolean[3]));
}
```

Passing the parent is what prevents a false positive — every edge you walk down is immediately walkable back up, and that is not a cycle.

For a **directed** graph the parent trick does not work. You need three states instead of a boolean: unvisited, currently on the stack, and finished. A cycle is an edge back to a node still on the stack.

```java
static final int UNVISITED = 0, IN_PROGRESS = 1, DONE = 2;

static boolean hasCycleDirected(int node, List<List<Integer>> adj, int[] state) {
    state[node] = IN_PROGRESS;

    for (int next : adj.get(node)) {
        if (state[next] == IN_PROGRESS) return true;      // back edge
        if (state[next] == UNVISITED && hasCycleDirected(next, adj, state)) {
            return true;
        }
    }
    state[node] = DONE;
    return false;
}

public static void main(String[] args) {
    List<List<Integer>> adj = Arrays.asList(
        Arrays.asList(1),
        Arrays.asList(2),
        Arrays.asList(0)
    );
    int[] state = new int[3];
    boolean cycle = false;
    for (int i = 0; i < 3; i++) {
        if (state[i] == UNVISITED && hasCycleDirected(i, adj, state)) {
            cycle = true;
        }
    }
    System.out.println(cycle);
}
```

A plain `boolean[]` cannot distinguish "on the current path" from "finished elsewhere", and would report cycles that do not exist.

### Iterative DFS

Recursion depth scales with the graph, so a large graph can overflow the stack. An explicit stack removes the limit:

```java
static void dfsIterative(int start, List<List<Integer>> adj, boolean[] visited) {
    Deque<Integer> stack = new ArrayDeque<>();
    stack.push(start);

    while (!stack.isEmpty()) {
        int node = stack.pop();
        if (visited[node]) continue;      // may have been queued twice
        visited[node] = true;

        for (int neighbour : adj.get(node)) {
            if (!visited[neighbour]) {
                stack.push(neighbour);
            }
        }
    }
}

public static void main(String[] args) {
    List<List<Integer>> adj = Arrays.asList(
        Arrays.asList(1, 2),
        Arrays.asList(0, 3),
        Arrays.asList(0),
        Arrays.asList(1)
    );
    dfsIterative(0, adj, new boolean[4]);
    System.out.println("done");
}
```

The `if (visited[node]) continue;` after popping is needed here. Unlike the recursive version, a node can be pushed several times before it is first popped, so you check on the way out rather than only on the way in.

### DFS or BFS

<ContentTable
  headers={["Question", "Use"]}
  rows={[
    ["Is everything connected?", "Either &mdash; DFS is shorter"],
    ["How many separate groups?", "Either"],
    ["Does a path exist between two nodes?", "Either"],
    ["Is there a cycle?", "DFS"],
    ["<strong>What is the shortest path?</strong>", "<strong>BFS</strong> &mdash; DFS gives <em>a</em> path, not the shortest"],
    ["Explore all possible arrangements", "DFS with undo &mdash; see backtracking"]
  ]}
/>

The shortest-path row is the one that catches people. DFS will happily return a long winding route when a direct one exists. If the question involves "fewest steps", use [breadth-first search](/java/java-algo-bfs).

<RulesBox title="Common mistakes">

- **Marking visited after recursing**, causing infinite recursion on a cycle.
- **No `visited` array at all** on a graph.
- **Starting from only one node**, missing disconnected parts.
- **Bounds check after array access** in grid code.
- **A boolean visited array for directed cycle detection** — you need three states.
- **Forgetting the parent** in undirected cycle detection, reporting every edge as a cycle.
- **Using DFS for shortest paths.**
- **Stack overflow** on a large graph — go iterative.

</RulesBox>

<JavaPlayground id="java-algo-dfs" />

<ExerciseBox title="Practice">

Fill in each starter, click Run, then open Hints if you get stuck.

1. Build a small undirected graph and print every node reachable from node 0.

```java
import java.util.*;

public class Main {
    static void addEdge(List<List<Integer>> adj, int u, int v) {
        adj.get(u).add(v);
        adj.get(v).add(u);
    }

    static void dfs(int node, List<List<Integer>> adj, boolean[] visited) {
        // TODO
    }

    public static void main(String[] args) {
        int n = 4;
        List<List<Integer>> adj = new ArrayList<List<Integer>>();
        for (int i = 0; i < n; i++) {
            adj.add(new ArrayList<Integer>());
        }
        addEdge(adj, 0, 1);
        addEdge(adj, 0, 2);
        addEdge(adj, 1, 3);

        boolean[] visited = new boolean[n];
        dfs(0, adj, visited);
        System.out.println(Arrays.toString(visited));
    }
}
```

2. Count the connected components of an undirected graph.

```java
import java.util.*;

public class Main {
    static void addEdge(List<List<Integer>> adj, int u, int v) {
        adj.get(u).add(v);
        adj.get(v).add(u);
    }

    static int countComponents(List<List<Integer>> adj, int n) {
        return 0;
    }

    public static void main(String[] args) {
        int n = 4;
        List<List<Integer>> adj = new ArrayList<List<Integer>>();
        for (int i = 0; i < n; i++) {
            adj.add(new ArrayList<Integer>());
        }
        addEdge(adj, 0, 1);
        addEdge(adj, 2, 3);

        System.out.println(countComponents(adj, n));
    }
}
```

3. Given a grid of `.` and `#`, count the regions of connected `.` cells.

```java
public class Main {
    static void fill(char[][] grid, boolean[][] seen, int r, int c) {
        // TODO
    }

    static int countRegions(char[][] grid) {
        boolean[][] seen = new boolean[grid.length][grid[0].length];
        return 0;
    }

    public static void main(String[] args) {
        char[][] grid = {
            {'.', '.', '#'},
            {'#', '.', '.'},
            {'.', '#', '.'}
        };
        System.out.println(countRegions(grid));
    }
}
```

4. Determine whether an undirected graph contains a cycle.

```java
import java.util.*;

public class Main {
    static void addEdge(List<List<Integer>> adj, int u, int v) {
        adj.get(u).add(v);
        adj.get(v).add(u);
    }

    static boolean hasCycle(int node, int parent, List<List<Integer>> adj, boolean[] visited) {
        return false;
    }

    public static void main(String[] args) {
        int n = 3;
        List<List<Integer>> adj = new ArrayList<List<Integer>>();
        for (int i = 0; i < n; i++) {
            adj.add(new ArrayList<Integer>());
        }
        addEdge(adj, 0, 1);
        addEdge(adj, 1, 2);
        addEdge(adj, 2, 0);

        System.out.println(hasCycle(0, -1, adj, new boolean[n]));
    }
}
```

5. Rewrite exercise 1 iteratively with an explicit stack and confirm the same set of nodes is reached.

```java
import java.util.*;

public class Main {
    static void addEdge(List<List<Integer>> adj, int u, int v) {
        adj.get(u).add(v);
        adj.get(v).add(u);
    }

    static void dfsIterative(int start, List<List<Integer>> adj, boolean[] visited) {
        Stack<Integer> stack = new Stack<Integer>();
        // TODO
    }

    public static void main(String[] args) {
        int n = 4;
        List<List<Integer>> adj = new ArrayList<List<Integer>>();
        for (int i = 0; i < n; i++) {
            adj.add(new ArrayList<Integer>());
        }
        addEdge(adj, 0, 1);
        addEdge(adj, 0, 2);
        addEdge(adj, 1, 3);

        boolean[] visited = new boolean[n];
        dfsIterative(0, adj, visited);
        System.out.println(Arrays.toString(visited));
    }
}
```


<details>
<summary>Hints</summary>

1. Adjacency list plus one DFS call. Remember to add edges in both directions.
2. The counting loop.
3. Flood fill, counting how many times you start a new fill.
4. Pass the parent down so the edge you arrived on is not mistaken for a cycle.
5. The order visited may differ from the recursive version — that is fine, only the set matters.

</details>

</ExerciseBox>

<LinkGrid
  title="Next"
  section="java"
  links={[
    {"label":"Breadth-First Search","id":"java-algo-bfs"},
    {"label":"Backtracking","id":"java-algo-backtracking"}
  ]}
/>

<LinkGrid
  title="Related"
  section="java"
  links={[
    {"label":"Graphs","id":"java-ds-graphs"},
    {"label":"Recursion","id":"java-algo-recursion"},
    {"label":"Stacks","id":"java-ds-stacks"}
  ]}
/>
