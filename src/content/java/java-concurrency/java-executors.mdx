...
A pool in teleop is the same smell as a raw `Thread`. Prefer the framework loop. A pool is for off-loop work such as image processing, not motor output. `newVirtualThreadPerTaskExecutor()` is Java 21+ and will not compile in this playground (Java 8).
