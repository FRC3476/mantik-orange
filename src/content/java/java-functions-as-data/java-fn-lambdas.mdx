...
```java
List<String> names = new ArrayList<>(Arrays.asList("intake", "drive", "shooter"));
...
```java
class Intake {
    void run() { System.out.println("intake running"); }
}

class Button {
    void whenPressed(Runnable action) { action.run(); }
}
...
class Command {
    void until(BooleanSupplier done) {
        System.out.println("finished: " + done.getAsBoolean());
    }
}
...
System.out.println(test.matches(150));
```
...
- **The body is long.** A lambda spanning fifteen lines is harder to read than a named method. Extract it and use a [method reference](/java/java-fn-method-references).
...
public class Main {
    public static void main(String[] args) {
        ...
        // TODO: sort by length, then alphabetically among equal lengths
...
```java
public class Main {
    static void repeat(Runnable r) {
    }

    public static void main(String[] args) {
        repeat(() -> { }); // TODO: print a counter
    }
}
```
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(test.matches(15));
        System.out.println(test.matches(5));
    }
}
```
...
3. `list.removeIf(s -> s.trim().isEmpty())`.
