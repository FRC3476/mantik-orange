...
Lambdas in `new Thread(() -> …)` come from [Lambda Expressions](/java/java-fn-lambdas). An anonymous `Runnable` (as in the Check) is enough if you skipped that module.

The FTC/FRC loop already *is* the concurrency model. Extra threads are for work that cannot live in that loop (for example a vision pipeline). Do not start threads that touch motors.
...
You will see both `worker` and `main` lines. The exact order is not guaranteed, and **it can change between runs**. That unpredictability is the defining feature of concurrent code, and the source of most of its difficulty.

The short `sleep` pauses give the other thread a chance to run. Without a pause, a tiny loop often finishes on one thread before the other starts.
...
Thread fromTask = new Thread(task);

// 3. extending Thread — rarely the right choice
Worker fromSubclass = new Worker();
...
Thread worker = new Thread(Work::heavyCalculation);

worker.start();
Work.doSomethingElse();       // happens concurrently
...
- **Shared state.** Two threads touching the same variable can corrupt it, even for operations that look atomic. See [Race Conditions](/java/java-race-conditions).
...
Thread background = new Thread(Sensor::pollSensor);
...
- **Creating threads in a loop.** Each carries real cost; use a pool. See [Executors and Thread Pools](/java/java-executors).
...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        worker.start();
        // TODO: worker.join() here so A–E wait until 1–5 finish
        for (char c = 'A'; c <= 'E'; c++) {
...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        worker.start();
        // Change start() to run() and compare.
        for (char c = 'A'; c <= 'E'; c++) {
            System.out.println(c);
        }
        worker.join();
