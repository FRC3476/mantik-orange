...
<RulesBox title="Browser practice">

Open [PID Simulation](/pid-simulation) on Mantik for a zero-install first pass. Choose **Flywheel**, pick REV or CTRE to match your hardware, and tune with SpringTune while watching wheel RPM on TraceView. Setpoint and gains use **motor rot/s** in code (same as SparkMax / Talon FX velocity mode); the graph shows **wheel RPM**.
...
- Units: Velocity graph shows rotations per second, setpoint may use degrees per second (convert with ×360). Phoenix Tuner and REV Hardware Client often display rotations per second, but the configured signal's units win — verify what that graph is actually plotting.
- After velocity PID holds, [Shooter Shot Table](/frc/shooter-shot-table) selects flywheel **output RPS** (and hood degrees) from field distance. This YAMS practice path may expose angular velocity in degrees per second. Convert explicitly: RPS = deg/s ÷ 360, RPM = RPS × 60. Do not paste a deg/s number into a Talon FX velocity request.
...
<LinkGrid links={[{"label":"Shooter Example","id":"shooter-example"},{"label":"Shooter Shot Table","id":"shooter-shot-table"},{"label":"PID Control","id":"frc-pid-control"},{"label":"PID Tuning Practice - Setup","id":"pid-tuning-practice-setup"},{"label":"PID Tuning Practice - Arm","id":"pid-tuning-practice-arm"},{"label":"PID Tuning Practice - Elevator","id":"pid-tuning-practice-elevator"},{"label":"WPILib: Feedforward Control","url":"https://docs.wpilib.org/en/stable/docs/software/advanced-controls/controllers/feedforward.html"}]} title="Resources" section="frc" />
