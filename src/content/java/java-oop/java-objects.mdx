...
class FRCRobot {
    ...
    FRCRobot(String name, int teamNumber) {
        this.name = name;
    ...
    void runDiagnostics() {
        System.out.println(name + " diagnostics: battery " + batteryLevel + "%");
    }
    public String toString() {
        return name + " (Team " + teamNumber + ") battery " + batteryLevel
...
class FRCRobot {
    ...
    void setBatteryLevel(double level) { batteryLevel = level; }
    void setLiftCapacity(int capacity) { liftCapacity = capacity; }
    void enablePrecisionMode(boolean enabled) { precisionMode = enabled; }
    void activate() {
        active = true;
        System.out.println(name + " activated");
    }
    void moveForward(int distance) {
        System.out.println(name + " moving forward " + distance + " at speed " + maxSpeed);
    }
    void liftObject() {
        System.out.println(name + " lifting with capacity " + liftCapacity);
    }
    ...
    public String toString() {
        return name + " (Team " + teamNumber + ") battery " + batteryLevel
            + "% speed " + maxSpeed + (active ? " active" : " inactive");
    }
}
...
class FRCRobot {
    ...
    FRCRobot(String name, int teamNumber) { this.name = name; }

    String getName() { return name; }
    void setBatteryLevel(double level) { batteryLevel = level; }
    double getBatteryLevel() { return batteryLevel; }
    void setMaxSpeed(int speed) { maxSpeed = speed; }
    void enableCompetitionMode(boolean enabled) { }
    void enableSafetyLimits(boolean enabled) { }
    void activate() { active = true; }
    boolean isActive() { return active; }
    void returnToBase() { System.out.println(name + " returning to base"); }
}
...
FRCRobot alpha = new FRCRobot("Alpha", 1);
FRCRobot beta = new FRCRobot("Beta", 2);
alpha.setBatteryLevel(20);
beta.setBatteryLevel(80);
manager.initializeRobot(alpha, "competition");
manager.initializeRobot(beta, "practice");
manager.compareRobots(alpha, beta);
manager.manageRobotTeam(new FRCRobot[] { alpha, beta, null });
```
...
1. Write a Robot class with a name. Create two and print both names.
...
public class Main {
    public static void main(String[] args) {
        Robot a = new Robot("Alpha");
        Robot b = new Robot("Beta");
...
2. Write activate(Robot r) that prints the name; call it twice.
...
public class Main {
    static void activate(Robot r) {
        // TODO: print the robot's name
    }
...
3. Store three robots in an array; loop and print names.
...
public class Main {
    public static void main(String[] args) {
        Robot a = new Robot("Alpha");
        Robot b = new Robot("Beta");
        Robot c = new Robot("Gamma");
        Robot[] team = new Robot[3];
        // TODO: store all three in the array, then loop and print names
    }
}
```

4. Put null in one slot and skip it safely.
...
public class Main {
    public static void main(String[] args) {
        Robot[] team = new Robot[3];
        team[0] = new Robot("Alpha");
        team[1] = null;
        team[2] = new Robot("Gamma");
        // TODO: loop and print names, skipping null slots
    }
}
```

5. Print whether two robots are the same object (`==`) vs same name.
...
public class Main {
    public static void main(String[] args) {
        Robot a = new Robot("Alpha");
        Robot b = new Robot("Alpha");
        Robot same = a;
        // TODO: print a == b
        // TODO: print a == same
        // TODO: print whether a and b have the same name
    }
}
```
...
1. The constructor should assign `this.name`. Main already prints both objects.
2. Use the parameter `r` inside `activate`. Main already calls it twice.
3. `team[0] = a;` and so on, then a `for` loop over `team`.
4. Check `team[i] != null` before you print.
5. `==` tests the same object in memory. Compare names with `.equals`.
