...
2. Deploy once.
3. Connect AdvantageScope to the robot and turn on tuning mode — the slider icon right of the search bar.
...
Step 5 is where logging discipline pays off. If your setters return the value they set and carry `@AutoLogOutput` — see [Subsystems](/frc/subsystems) — both halves of the comparison are already recorded:
...
The numbers below are **illustrative**. Keys are meters. Values are flywheel output rotations per second.

```java norun
import edu.wpi.first.math.interpolation.InterpolatingDoubleTreeMap;

InterpolatingDoubleTreeMap flywheelRps = new InterpolatingDoubleTreeMap();
flywheelRps.put(1.790, 40.00);
flywheelRps.put(1.900, 41.50);
flywheelRps.put(2.050, 42.00);
flywheelRps.put(2.200, 43.00);
flywheelRps.put(2.558, 44.50);
flywheelRps.put(4.947, 61.75);

double rps = flywheelRps.get(distanceM);
```

`get` returns a stored value at an exact key and linearly interpolates between keys. Outside the keys it returns the nearest endpoint. An empty map returns `null`.

That endpoint clamp is **not** a validated shooting range. A distance below 1.790 m still gets 40.00 RPS from the raw map; that does not mean the shot will score. Interpolation is exact at stored keys. Between keys it is an estimate, not a physics proof.

A fitted curve is a different model. It also needs measured shots to validate. Do not assume it extrapolates safely past the distances you tested.

A hand-written loop that interpolates an array pair is easy to get wrong: if the last key is an exact match and the loop only fires when `distance[i] > d`, the method falls through and returns 0. Use `InterpolatingDoubleTreeMap`, then wrap it with an inclusive measured-range check.

The full paired table (flywheel RPS **and** hood degrees), alliance goal selection, and feed interlock are in [Shooter Shot Table](/frc/shooter-shot-table).
...
<LinkGrid links={[{"label":"Shooter Shot Table","id":"shooter-shot-table"},{"label":"PID Control","id":"frc-pid-control"},{"label":"Advanced Control","id":"advanced-control-theory"},{"label":"PID Tuning Practice","id":"pid-tuning-practice-setup"},{"label":"AdvantageScope","id":"advantagescope"},{"label":"WPILib System Identification","url":"https://docs.wpilib.org/en/stable/docs/software/advanced-controls/system-identification/index.html"}]} title="Resources" section="frc" />
