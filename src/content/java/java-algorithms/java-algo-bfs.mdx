---
title: Breadth-First Search
lessonId: java-algo-bfs
section: java
group: java-algorithms
groupLabel: "Common Algorithms"
groupOrder: 6
order: 6
difficulty: intermediate
duration: 30 min
description: Exploring outward in rings from a starting point, why that finds shortest paths, and how to recover the route rather than just the distance.
---

<RulesBox title="Optional later">

You need [Queues](/java/java-ds-queues) and [Graphs](/java/java-ds-graphs). BFS is how you get fewest steps on an unweighted grid — useful later for path ideas, not for first teleop.

</RulesBox>

### Spreading outward

Breadth-first search visits everything one step away, then everything two steps away, and so on. It uses a [queue](/java/java-ds-queues) rather than recursion, which is what makes it spread sideways instead of diving deep.

```java
static int[] bfs(int start, List<List<Integer>> adj, int n) {
    int[] distance = new int[n];
    Arrays.fill(distance, -1);           // -1 means not reached yet

    Queue<Integer> queue = new ArrayDeque<>();
    distance[start] = 0;
    queue.offer(start);

    while (!queue.isEmpty()) {
        int node = queue.poll();

        for (int neighbour : adj.get(node)) {
            if (distance[neighbour] == -1) {
                distance[neighbour] = distance[node] + 1;
                queue.offer(neighbour);
            }
        }
    }
    return distance;
}

public static void main(String[] args) {
    int n = 4;
    List<List<Integer>> adj = Arrays.asList(
        Arrays.asList(1, 2),
        Arrays.asList(0, 3),
        Arrays.asList(0),
        Arrays.asList(1)
    );
    System.out.println(Arrays.toString(bfs(0, adj, n)));
}
```

`distance` doubles as the visited marker, so no separate array is needed.

### Why it finds shortest paths

Because nodes come out of the queue in order of distance, the first time you reach a node is by the fewest possible edges. There is no shorter route still to be discovered — anything shorter would have been dequeued earlier.

This is the property [depth-first search](/java/java-algo-dfs) lacks. DFS reaches nodes in whatever order its recursion happens to take, so its first arrival is not necessarily the shortest.

<RulesBox title="Set the distance when you enqueue, not when you dequeue">

```java
int[] distance = new int[3];
Arrays.fill(distance, -1);
Queue<Integer> queue = new ArrayDeque<>();
distance[0] = 0;
queue.offer(0);

int node = 0;
int neighbour = 1;
distance[neighbour] = distance[node] + 1;
queue.offer(neighbour);                    // both together

System.out.println(Arrays.toString(distance));
```

If you only record the distance after polling, the same node can be offered several times before it is first processed. The queue fills with duplicates, the search slows dramatically, and on large graphs it can exhaust memory.

Marking on discovery guarantees each node enters the queue exactly once.

</RulesBox>

BFS gives shortest paths only when **every edge counts the same**. With varying edge costs — distances, travel times, fuel — the fewest-edges route may not be the cheapest, and you need Dijkstra's algorithm instead.

### On a grid

Most BFS you write will be on a grid, where neighbours are computed rather than stored.

```java
static int shortestPath(char[][] grid, int startR, int startC, int endR, int endC) {
    int rows = grid.length, cols = grid[0].length;
    int[][] dist = new int[rows][cols];
    for (int[] row : dist) Arrays.fill(row, -1);

    int[] dr = {-1, 1, 0, 0};
    int[] dc = {0, 0, -1, 1};

    Queue<int[]> queue = new ArrayDeque<>();
    dist[startR][startC] = 0;
    queue.offer(new int[]{startR, startC});

    while (!queue.isEmpty()) {
        int[] cell = queue.poll();
        int r = cell[0], c = cell[1];

        if (r == endR && c == endC) return dist[r][c];   // arrived

        for (int d = 0; d < 4; d++) {
            int nr = r + dr[d], nc = c + dc[d];

            if (nr < 0 || nr >= rows || nc < 0 || nc >= cols) continue;
            if (grid[nr][nc] == '#' || dist[nr][nc] != -1) continue;

            dist[nr][nc] = dist[r][c] + 1;
            queue.offer(new int[]{nr, nc});
        }
    }
    return -1;                            // unreachable
}

public static void main(String[] args) {
    char[][] grid = {
        {'.', '.', '#'},
        {'.', '.', '.'},
        {'#', '.', '.'}
    };
    System.out.println(shortestPath(grid, 0, 0, 2, 2));
}
```

Storing coordinates as a two-element `int[]` avoids defining a class. Bounds checks come before reading the grid, as always.

### Recovering the path

Distance alone often is not the answer — you may need the actual route. Record where each node was first reached from:

```java
int n = 4;
int[] distance = new int[n];
Arrays.fill(distance, -1);
int[] parent = new int[n];
Arrays.fill(parent, -1);

List<List<Integer>> adj = Arrays.asList(
    Arrays.asList(1, 2),
    Arrays.asList(0, 3),
    Arrays.asList(0),
    Arrays.asList(1)
);

Queue<Integer> queue = new ArrayDeque<>();
distance[0] = 0;
queue.offer(0);

while (!queue.isEmpty()) {
    int node = queue.poll();
    for (int neighbour : adj.get(node)) {
        if (distance[neighbour] == -1) {
            distance[neighbour] = distance[node] + 1;
            parent[neighbour] = node;             // remember how we got here
            queue.offer(neighbour);
        }
    }
}
System.out.println(Arrays.toString(parent));
```

Then walk backwards from the destination and reverse:

```java
static List<Integer> pathTo(int target, int[] parent) {
    List<Integer> path = new ArrayList<>();
    for (int at = target; at != -1; at = parent[at]) {
        path.add(at);
    }
    Collections.reverse(path);
    return path;
}

public static void main(String[] args) {
    int[] parent = {-1, 0, 0, 1};        // 0 → 1 → 3, and 0 → 2
    System.out.println(pathTo(3, parent));
}
```

If the target was never reached its parent is still `-1`, so check the distance before trusting the path.

### Multi-source BFS

Sometimes you want the distance from the *nearest* of several starting points — how far each cell is from any exit, say. Rather than running BFS once per source, seed the queue with all of them at distance 0:

```java
char[][] grid = {
    {'.', '.', '#'},
    {'.', '.', '.'},
    {'#', '.', '.'}
};
int rows = grid.length, cols = grid[0].length;
int[][] dist = new int[rows][cols];
for (int[] row : dist) Arrays.fill(row, -1);

int[] dr = {-1, 1, 0, 0};
int[] dc = {0, 0, -1, 1};

Queue<int[]> queue = new ArrayDeque<>();
int[][] sources = {{0, 0}, {2, 2}};

for (int[] source : sources) {
    dist[source[0]][source[1]] = 0;
    queue.offer(source);
}

while (!queue.isEmpty()) {
    int[] cell = queue.poll();
    int r = cell[0], c = cell[1];
    for (int d = 0; d < 4; d++) {
        int nr = r + dr[d], nc = c + dc[d];
        if (nr < 0 || nr >= rows || nc < 0 || nc >= cols) continue;
        if (grid[nr][nc] == '#' || dist[nr][nc] != -1) continue;
        dist[nr][nc] = dist[r][c] + 1;
        queue.offer(new int[]{nr, nc});
    }
}

for (int[] row : dist) {
    System.out.println(Arrays.toString(row));
}
```

Everything else is unchanged, and one pass gives every cell its distance to the closest source. This is a genuinely useful trick that is not obvious the first time.

### Level by level

To know which "ring" you are processing, capture the queue size before draining it:

```java
List<List<Integer>> adj = Arrays.asList(
    Arrays.asList(1, 2),
    Arrays.asList(0, 3),
    Arrays.asList(0),
    Arrays.asList(1)
);
Queue<Integer> queue = new ArrayDeque<>();
boolean[] visited = new boolean[4];
visited[0] = true;
queue.offer(0);

int steps = 0;
while (!queue.isEmpty()) {
    int levelSize = queue.size();         // fixed before adding the next ring

    for (int i = 0; i < levelSize; i++) {
        int node = queue.poll();
        System.out.print(node + " ");
        for (int neighbour : adj.get(node)) {
            if (!visited[neighbour]) {
                visited[neighbour] = true;
                queue.offer(neighbour);
            }
        }
    }
    System.out.println("(distance " + steps + ")");
    steps++;
}
```

Reading `queue.size()` directly in the inner condition would include nodes added during the loop, merging the levels.

<ContentTable
  title="BFS or DFS"
  headers={["Need", "Use"]}
  rows={[
    ["Fewest steps / shortest path (equal costs)", "BFS"],
    ["Just reach everything", "Either"],
    ["Cycle detection", "DFS"],
    ["Explore all arrangements", "DFS with undo"],
    ["Distance from nearest of several sources", "Multi-source BFS"],
    ["Varying edge costs", "Neither &mdash; Dijkstra"]
  ]}
/>

<RulesBox title="Common mistakes">

- **Marking visited on dequeue instead of enqueue**, filling the queue with duplicates.
- **Using BFS with varying edge weights** and expecting the cheapest route.
- **Reading `queue.size()` inside the level loop.**
- **Bounds check after grid access.**
- **Using `LinkedList` as the queue** — `ArrayDeque` is faster.
- **Forgetting the unreachable case**, and treating `-1` as a real distance.
- **Using DFS when the question says "shortest" or "fewest".**

</RulesBox>

<JavaPlayground id="java-algo-bfs" />

<ExerciseBox title="Practice">

Fill in each starter, click Run, then open Hints if you get stuck.

1. Given an unweighted graph, print the distance from node 0 to every other node.

```java
import java.util.*;

public class Main {
    static void addEdge(List<List<Integer>> adj, int u, int v) {
        adj.get(u).add(v);
        adj.get(v).add(u);
    }

    public static void main(String[] args) {
        int n = 4;
        List<List<Integer>> adj = new ArrayList<List<Integer>>();
        for (int i = 0; i < n; i++) {
            adj.add(new ArrayList<Integer>());
        }
        addEdge(adj, 0, 1);
        addEdge(adj, 0, 2);
        addEdge(adj, 1, 3);

        int[] dist = new int[n];
        Arrays.fill(dist, -1);
        // TODO: BFS from 0, filling dist

        System.out.println(Arrays.toString(dist));
    }
}
```

2. Given a grid with walls, print the fewest steps from the top-left to the bottom-right, or `-1`.

```java
import java.util.*;

public class Main {
    static int shortestPath(char[][] grid) {
        int rows = grid.length;
        int cols = grid[0].length;
        int[][] dist = new int[rows][cols];
        for (int r = 0; r < rows; r++) {
            Arrays.fill(dist[r], -1);
        }
        // TODO: BFS from (0, 0) to (rows - 1, cols - 1)
        return -1;
    }

    public static void main(String[] args) {
        char[][] grid = {
            {'.', '.', '#'},
            {'.', '.', '.'},
            {'#', '.', '.'}
        };
        System.out.println(shortestPath(grid));
    }
}
```

3. Extend exercise 2 to print the actual route.

```java
import java.util.*;

public class Main {
    public static void main(String[] args) {
        char[][] grid = {
            {'.', '.', '#'},
            {'.', '.', '.'},
            {'#', '.', '.'}
        };
        int rows = grid.length;
        int cols = grid[0].length;
        int[][] parentR = new int[rows][cols];
        int[][] parentC = new int[rows][cols];
        for (int r = 0; r < rows; r++) {
            Arrays.fill(parentR[r], -1);
            Arrays.fill(parentC[r], -1);
        }
        // TODO: BFS from (0, 0), recording parent coordinates

        System.out.println("TODO path");
    }
}
```

4. Given a grid with several exits, print each open cell's distance to the nearest exit.

```java
import java.util.*;

public class Main {
    public static void main(String[] args) {
        char[][] grid = {
            {'E', '.', '#'},
            {'.', '.', '.'},
            {'#', '.', 'E'}
        };
        int rows = grid.length;
        int cols = grid[0].length;
        int[][] dist = new int[rows][cols];
        for (int r = 0; r < rows; r++) {
            Arrays.fill(dist[r], -1);
        }
        // TODO: multi-source BFS from every 'E'

        for (int r = 0; r < rows; r++) {
            System.out.println(Arrays.toString(dist[r]));
        }
    }
}
```

5. Print a graph's nodes grouped by distance from the start, one line per distance.

```java
import java.util.*;

public class Main {
    static void addEdge(List<List<Integer>> adj, int u, int v) {
        adj.get(u).add(v);
        adj.get(v).add(u);
    }

    public static void main(String[] args) {
        int n = 4;
        List<List<Integer>> adj = new ArrayList<List<Integer>>();
        for (int i = 0; i < n; i++) {
            adj.add(new ArrayList<Integer>());
        }
        addEdge(adj, 0, 1);
        addEdge(adj, 0, 2);
        addEdge(adj, 1, 3);

        Queue<Integer> queue = new ArrayDeque<Integer>();
        boolean[] visited = new boolean[n];
        // TODO: BFS, printing one line of nodes per distance
    }
}
```


<details>
<summary>Hints</summary>

1. The basic BFS; unreached nodes keep `-1`.
2. The grid version above.
3. Add a parent array storing coordinates, then walk back from the end.
4. Multi-source: seed every exit at distance 0 before the loop.
5. The level-size capture.

</details>

</ExerciseBox>

<LinkGrid
  title="Next"
  section="java"
  links={[
    {"label":"Backtracking","id":"java-algo-backtracking"},
    {"label":"Memoization and Dynamic Programming","id":"java-algo-memoization"}
  ]}
/>

<LinkGrid
  title="Related"
  section="java"
  links={[
    {"label":"Depth-First Search","id":"java-algo-dfs"},
    {"label":"Queues","id":"java-ds-queues"},
    {"label":"Graphs","id":"java-ds-graphs"}
  ]}
/>
