...
class Drivetrain extends Subsystem {
    @Override
    void periodic() { System.out.println("updating drive motors"); }
}
...
class Intake extends Subsystem {
    @Override
    void periodic() { System.out.println("checking intake sensor"); }
}

List<Subsystem> subsystems = Arrays.asList(new Drivetrain(), new Intake());
...
```java
class Subsystem {
    void periodic() { System.out.println("base periodic"); }
}

class Intake extends Subsystem {
    void periodick() { System.out.println("intake periodic"); }      // typo — compiles fine, never runs
}

Subsystem s = new Intake();
s.periodic();
```
...
class Drivetrain extends Subsystem implements Stoppable {
    @Override public void periodic() { System.out.println("updating drive motors"); }
...
Only overriding is polymorphism. See [Method and Constructor Overloading](/java/java-overloading) for the other one.
...
class Drivetrain extends Subsystem implements Stoppable {
    @Override void periodic() { System.out.println("updating drive motors"); }
    @Override public void stop() { System.out.println("brakes on"); }
}

class Intake extends Subsystem {
    @Override void periodic() { System.out.println("checking intake sensor"); }
}

List<Subsystem> subsystems = Arrays.asList(new Drivetrain(), new Intake());
for (Subsystem s : subsystems) {
    if (s instanceof Stoppable) {            // Java 8; Java 16+ can bind a variable here
        Stoppable stoppable = (Stoppable) s;
        stoppable.stop();
    }
}
```
...
class Drivetrain extends Subsystem {
    void driveForward() { System.out.println("driving forward"); }
}

class Intake extends Subsystem {
    void runIntake() { System.out.println("running intake"); }
}
...
public class Main {
    public static void main(String[] args) {
        List<Shape> shapes = Arrays.asList(new Circle(3), new Rectangle(4, 5));
        for (Shape s : shapes) {
...
class Circle extends Shape {
    ...
    // TODO: override describe in Circle only
}
...
public class Main {
    public static void main(String[] args) {
        List<Object> items = Arrays.asList(new Intake(), new Sensor(), new Camera());
        for (Object item : items) {
            // TODO: stop only if item is Stoppable
        }
        System.out.println("checked " + items.size());
    }
}
```
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (Animal a : animals) {
            handle(a);
            // TODO: replace handle(a) with a.act()
        }
        System.out.println("rewrite using act()");
    }
}
```
