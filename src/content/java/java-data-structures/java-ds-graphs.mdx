...
This page is how to *store* connections. Searching them is [Depth-First Search](/java/java-algo-dfs) and [Breadth-First Search](/java/java-algo-bfs). First-year teleop does not need graphs.

</RulesBox>

### Dropping the hierarchy

A [tree](/java/java-ds-trees) has a root, and every node has exactly one parent. A graph drops both rules. Any node may connect to any other, connections can form loops, and there is no top.
...
```java
static void addEdge(Map<String, List<String>> adj, String a, String b) {
    adj.computeIfAbsent(a, k -> new ArrayList<String>()).add(b);
    adj.computeIfAbsent(b, k -> new ArrayList<String>()).add(a);
}

public static void main(String[] args) {
    Map<String, List<String>> adj = new HashMap<>();
    addEdge(adj, "intake", "shooter");
    addEdge(adj, "shooter", "hood");
    System.out.println(adj);
}
```
...
Explore all immediate neighbours first, then their neighbours, spreading outward in rings. Uses a [queue](/java/java-ds-queues).
...
public class Main {
    ...
    static void dfs(int node, List<List<Integer>> adj, boolean[] visited) {
        // TODO: print every node reachable from here
    }
...
public class Main {
    ...
    static int fewestEdges(List<List<Integer>> adj, int n, int start, int goal) {
        return -1;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(fewestEdges(adj, n, 0, 3));
    }
}
```
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(reachable(adj, 0, 2));
    }
}
```
...
<LinkGrid
  ...
  section="comp"
  links={[
    {"label":"Depth-First Search","url":"/java/java-algo-dfs"},
